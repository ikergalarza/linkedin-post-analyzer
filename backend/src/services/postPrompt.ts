...
8) "Destripo / pantallazo de perfil de LinkedIn ilustrado" (Unai) — 0.2x.
   A generic illustration of a LinkedIn profile. It neither opens a curiosity gap nor proves anything documentary nor delivers a bodily-change joke → no engine → image had no motor. (Note: the CONCEPT "desmonta/destripo perfiles" itself is NOT the problem — same concept was 9.13x on Iker. This Unai flop had TWO likely executional issues, not a dead format: the image had no motor AND the hook verb "destripo" probably overshot (too extreme/visceral) where Iker's "desmonto" was the sweet spot — see § 3.6 for the verb diagnosis + the VERB INTENSITY CEILING.)
...
- Image likely also contributed (Unai's paired a generic profile illustration with no motor — see the IMAGE note). The hook verb is the most actionable lever, but treat this as a TENTATIVE read (the account/network variable is still unisolated), not a proven law.
...
When the user asks for a LEAD MAGNET: write the closer with the EXPLICIT "Comenta X + Y" formula (X = resource word, Y = low-effort detail like the reader's sector). DO NOT use the deprecated implicit phrasings ("dímelo abajo", "levanto la mano", "X aquí abajo") — they tanked our comment count in two independent A/B tests. Also: ask out loud whether the deliverable can be done LIVE in comments (audit / roast / one-liner advice) — if yes, propose live-delivery (9.13x precedent) before defaulting to a PDF. BUT FIRST sanity-check the live deliverable gives the commenter REAL VALUE (a critique / advice / teardown they want) — NOT a joke at their expense or with no payoff (the "traduzco al idioma del vendedor más cringe" live-reply lead magnet flopped at 1.1K imp for exactly this; see § 3). Then check whether the account has shipped a lead magnet in the last 7–10 days; if yes → flag the fatigue risk and suggest a different mechanic before drafting.
