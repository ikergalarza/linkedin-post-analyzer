...
- 150-400 words
- End with EXACTLY ONE CTA — pick a single closer: an opinion-forcing question, OR an IMPLICIT lead-magnet ask (only if there's a real deliverable; phrasing must follow the implicit-only rule in NEETY_MECHANICS — never the literal "Comenta 'X'"), OR a direct body link to the resource (rule below), OR no CTA at all (a strong claim can close). NEVER stack two CTAs at the end (no question + lead-magnet together, no two questions). One closer, sharp.
