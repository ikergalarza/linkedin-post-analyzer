...
1. **HOOK — disparador + tema de IA en tendencia.** Alarma (`🚨 ÚLTIMA HORA` / `⚰️ D.E.P.` / `BREAKING:` / bold + `🔥`) sobre lo ÚLTIMO de IA (skills, agentes, un modelo nuevo). El sujeto es el TRABAJO del lector (`§4.5.0`), no el modelo. Ej. real Martina: *"⚰️ D.E.P. prospección manual"*; Guillermo: *"BREAKING: [modelo] just made [X] accessible"* y *"I turned McKinsey frameworks into AI Skills 🔥"*.
