# SKILL: global-instructions — Cómo se hace un post que funciona (todos los aprendizajes)

> **name:** global-instructions
> **description:** El manual completo de creación de posts de LinkedIn para Neety. Reglas de hooks, cuerpos, formateado, mecánicas virales, imágenes y diagnóstico, destiladas de meses de publicación real. Es el QUÉ y el CÓMO-mecánico de cada post.
> **when-to-use:** Cargar SIEMPRE que se escriba, edite o critique una publicación. Se apoya en `aboutme` (identidad), `brand-voice` (tono), `working-preferences` (flujo) y `outliers-database` (datos vivos de arquetipos).
...
- **Un post GANA** cuando hace dos cosas a la vez: (a) **explica cómo funciona de verdad un sistema oculto** (un mecanismo, un incentivo, una regla de distribución que la mayoría no ve) Y (b) **rompe una creencia comúnmente aceptada** en las primeras 1-3 líneas.
- **Un post PIERDE** cuando cambias el nombre del autor por otro del nicho y sigue siendo igual de creíble. Si lo podría haber escrito cualquiera → demasiado genérico.

El test se aplica **antes** de escribir, no al final. Si no hay creencia que romper ni mecanismo que enseñar, edita la idea hasta que aparezca o pasa a otra.

**Nunca produzcas:** posts genéricos que cualquiera del nicho podría publicar; recaps de noticias de IA sin ángulo propio; opiniones sin prueba (número, anécdota, caso); posts "personales" sin edge.

**Antídoto contra lo genérico (usar en cuerpos):** cuando el post toca cómo vende Neety o el dolor del comprador, aterrízalo en los **3 pilares diferenciales** y los **contrastes concretos** de `aboutme` §1b (listado con confianza vs lista; contacto por señal vs secuencia ciega; el comercial valida y enseña vs "lo hace por ti"; señales concretas: LinkedIn, cambio de puesto, nueva planta, feria). Eso es lo que hace que el cuerpo NO se pueda copiar-pegar a cualquier competidor. Ojo con el encuadre del público (industria y servicios = a quién le hablamos; los leads que generamos son del sector que sea).

---

## 1 · Los 3 pilares psicológicos (elige UNO y llévalo al extremo)

Cada post compromete UN pilar dominante y usa SOLO las palancas de ese pilar. Tocar los tres a la vez diluye. Trazas de los otros están bien; activar los tres, no.

- **CURIOSIDAD:** information gap + "nadie lo cuenta"/insider, reversal ("todos dicen X — en realidad Y"), loop abierto en el corte (`…`, `:`, `👇`), detalle clave retenido hasta el cuerpo, setup de plot-twist.
- **MIEDO:** obsolescencia ("X ha muerto"), riesgo de reemplazo ("un agente lo hace por 5€"), bandwagon en contra ("todos migran a X y tú no"), stakes concretos ("pierdes N% de pipeline"), cuenta atrás / "antes de que sea tarde".
- **DESEO:** cifras de dinero ($, €, MRR, ARR), velocidad ("en 60 segundos", "en 7 días"), promesa de sistema/framework/playbook, status / top-1%, antes→después con resultado concreto.

Si el hook toma prestada la palanca de otro pilar → reescribe. La mezcla de pilares es la dilución #1.
...
- El hook es **UN bloque.** Una frase, o dos frases cortas, pero **en el mismo párrafo** — sin `\n\n` entre ellas — y ≤ 210 caracteres.
...
**🗣️ EL REFRÁN INVERTIDO: la otra vía, y trae la pegada ya hecha (Iker, 2026-08-05).** Además de la metáfora, funciona **coger un refrán que sabe hasta un niño y darle la vuelta**: `En ventas ya no la consigue el que la sigue` · `A quien madruga buscando clientes ya no le ayuda nadie`.
- **Por qué pega:** el lector conoce el final de memoria, y cuando se lo cambias **se para en seco**. El bucle no es "¿por qué dices eso?" sino **"¿entonces quién la consigue?"**, que es una pregunta que él QUIERE que le respondas. Y la segunda línea del cuerpo se la respondes.
- **⭐ Y hay un efecto extra que Iker detectó al leerlo en alto: la RIMA INTERNA.** `consigue` / `sigue` hace que el ojo tropiece y **vuelva atrás a releer**. Ese medio segundo es tiempo de permanencia, que es señal para el algoritmo. **No es casualidad: los refranes llevan rima o ritmo justamente porque están hechos para pegarse**, así que al usar uno te llevas ese trabajo ya hecho.
- **Los límites, que son dos:** el refrán tiene que ser **de los que sabe un niño** (nada de "a ojo de buen cubero" ni "poner puertas al campo", que ya son de otra generación), y **la vinculación a ventas no se pierde por meter el refrán**: si al invertirlo se queda sin `ventas`, `vender`, `cliente` o `comercial`, no vale.
...
- El lado **dinero/resultado**: cerrar clientes, cerrar un deal, facturar, cuota, comisión.
- El lado **cliente**: cliente / clientes / cartera / "el cliente que se te escapó".
...
Demasiado nicho para el hook (mándalo al CUERPO): "pipeline", "outbound", "cadencia", "reply rate", "touchpoints", "SDR/AE" como término, "cold email", "discovery", "GTM", "ICP", **"CRM"** y **"forecast"**. **Tampoco "B2B" en el hook**: describe bien a nuestro cliente pero estrecha el alcance. Dentro de VENTAS, cuanto más genérico el gancho, más lejos llega; la especificidad va en el cuerpo.
- **⭐ La regla vale para CUALQUIER palabra que encoja la audiencia, no solo la jerga de ventas (Iker, 2026-07-22).** Ej: **"webinar" fuera del hook** (solo interesa al que ya va a webinars) → en el hook, el sinónimo más amplio y con más autoridad (**"conferencia"**); "webinar" baja al cuerpo. Antes de fijar el hook, pregúntate: ¿esta palabra la busca/entiende cualquiera, o solo un nicho? Si es de nicho, al cuerpo.
...
- **Caso real:** un meme abría con *"El pedido encoge cada vez que sube un piso"*. El validador lo dio por bueno (`ancla: "pedido"`) y el usuario preguntó: **"¿la palabra pedido ya se sabe que es de ventas?"**. No se sabe: ese hook lo puede publicar un jefe de logística tal cual. Se arregló a *"**En ventas**, el pedido encoge…"*.
- **Mira lo que hacen los ganadores: lo dicen EXPLÍCITO.** *"**En ventas**, cada año la caja de herramientas engorda pero el **comercial** cierra menos"* (16.62x) · *"Subir **en ventas** siempre pasa factura"* (13.61x). Ninguno lo deja a la imaginación.
- **El validador ya lo canta** (`⚠️ ancla solo AMBIGUA`) pero **no falla**, y a propósito: Navarra (7.72x, el molde gold) ancla en "exporta", que es ambigua, y voló. Ambigua no es prohibida: es **candidata a revisión humana**. Si el hook cuela en otra cuenta, mete "ventas" o "comercial".
...
- **Nunca** abras el cuerpo con preámbulos de credencial/autoridad: "Llevamos meses…", "Después de X años…", "Tras analizar…", "He hablado con N equipos…".
- **Nunca** con anunciadores-explicadores: "La realidad es que…", "Lo cierto es que…", "Déjame explicarte…", "Te cuento…".
- El cuerpo **dispara en la línea 2**: descarga parcial del loop (5-12 palabras), un dato duro, una escena/diálogo, o una lista. Nunca un windup. La autoridad se prueba con el contenido, nunca se anuncia.

### 2.7 · Arquetipos de hook que más outliers producen
Ruptura de patrón · Rompe creencias · Dato impactante · Intriga (curiosity gap) · Confesión personal · Opinión polémica (hot take) · Promesa de lista · Llamada directa · How-to. (Etiquetas de la herramienta; los ratios reales por arquetipo están en `outliers-database`.)

### 2.8 · Openers/closers quemados (nunca literales)
...
## 3 · El CUERPO (formateado y ritmo)

### 3.1 · Enumeración y frases paralelas
Cuando tengas 2+ frases que sean estructura paralela, enumeración, dato+consecuencia-punchline o setup+dardo → **cada una en su propia línea**, separadas por un solo salto (`\n`), **nunca** pegadas en un párrafo con punto-espacio.
- ✅ `No es la inversión.\nNo es el espacio.\nNo son los mentores.`
...
- **⭐ UNA FRASE POR LÍNEA. Si hay un punto en medio, se corta ahí (Iker, 2026-07-27).** *"Yo eso no lo hago en la vida."* Meter dos frases en la misma línea (*"Buscar a mano no te hace mejor comercial. Solo te deja sin mañana."*) rompe el ritmo y delata que el bloque se montó a ojo. Cada punto es un salto de línea.
- **⭐ LA FRASE DE DESPUES DEL HOOK VA SUELTA, NUNCA EN BLOQUE (Iker, 2026-07-29).** Justo despues del gancho toca **una linea individual**. Un bloque ahi queda mal: el lector viene de abrir el "ver mas" y lo primero que se encuentra es un muro. El primer bloque va **despues** de esa linea suelta.
- **⭐ UN BLOQUE COMPARTE TEMA, no solo forma (Iker, 2026-07-29).** Dos frases seguidas no son un bloque por estar pegadas: **un bloque de 2 o de 3 habla de UNA sola cosa**, y por eso lleva anafora. Si las dos lineas van de temas distintos (el tono con el que te lo dijeron y lo que opinaba del producto), o se reescriben para compartir tema o se dejan sueltas. **Fallo real de ese dia:** yo pegue *"Sin maldad ninguna, que es lo que mas me marco"* con *"El producto le resolvia el problema"* — dos temas. Arreglado con anafora y un tema unico, lo que NO fue el problema: *"No tenia ni una pega del producto. Lo reconocio el." / "No lo dijo con mala intencion, que es lo que mas me marco."*
- **⭐ ESCALERA ASCENDENTE POR DEFECTO en el bloque de 2 (Iker, 2026-07-27):** primera línea **corta**, segunda **más larga**. La invertida sigue siendo válida (§3.2), pero cuando puedas elegir, sube. Ej. bueno: *"Buscar a mano no te hace mejor comercial."* (41) → *"Solo te deja sin mañana y sin una llamada hecha."* (48).

**⛔ EN EL MEME NO APLICA (resuelto por el usuario, 2026-07-17).** El par va en **mapa, "Los 10" y lead magnet**. El meme queda fuera, y no por conveniencia: **el meme CALCA su referencia y punto** (`post-workflow §4.3`), así que su forma la decide el post que estamos remixando, no nuestro ritmo. La evidencia iba por delante de la decisión: al meter el check, los 2 memes ganadores (13.51x y 16.46x) lo fallaron mientras el mapa de Navarra 7.72x lo pasaba. Se anotó el conflicto sin taparlo y el usuario lo cerró así. **Lección: cuando un check tumba un ganador, la pregunta no es cómo silenciarlo, es qué te está diciendo.**
...
- **ESCALERA: aplica a TODO bloque de 2 y de 3, y la longitud tiene que ser MONÓTONA.** Recta (cada línea más larga que la anterior) o **invertida** (cada línea más corta). Las dos valen y hay que **alternarlas** entre bloques: si el anterior subía, que el siguiente baje.
  - **⭐ Y LA SUBIDA TIENE QUE SER SUAVE, con los saltos parejos (Iker, 2026-07-27).** No basta con que sea monótona: si va `49 → 51 → 52 → 63 → 69` (saltos de +2, +1, +11, +6) se ve un escalón brusco en medio y canta. Reparte el recorrido: de la línea más corta a la más larga, con incrementos parecidos. El mismo caso arreglado: `49 → 55 → 59 → 63 → 69` (+6, +4, +4, +6). **Aplica también a la LISTA NUMERADA del lead magnet**, no solo a los bloques de prosa.
  - El fallo típico no es no hacer escalera: es el **zigzag**, que una del medio rompa la progresión. `46 → 41 → 54` no es escalera aunque la 1ª y la 3ª sí escalen. Caso real (Asturias, 2026-07-15): *"Se sabe el producto…" (46) / "Se sabe el nombre del hijo…" (41) / "Se hace 600 kilómetros…" (54)*. **Se arregló solo reordenando** (41 → 46 → 54), sin tocar una palabra: antes de reescribir, prueba a permutar las líneas.
  - **Está en el validador** (mide los caracteres de cada línea de cada bloque de prosa): es aritmética, no criterio, y aun así se coló con la regla ya escrita.
...
Un cuerpo que rinde alterna conscientemente entre 4 unidades; un post de 300-500 palabras usa AL MENOS 3 de las 4:
- **(a) Bloque compacto de 2 líneas** — cuando 2 frases son la MISMA idea (setup+reveal, antes+después, causa+consecuencia). Un solo salto, sin línea en blanco. Cadencia escalera: línea 1 más corta, línea 2 un poco más larga.
- **(b) Línea aislada** — para dardos, punchlines, reveals, transiciones. Línea en blanco antes Y después. El aislamiento ES el efecto.
- **(c) Enumeración vertical paralela** — 2+ frases con sintaxis repetida ("El SDR que… / El AE que…"). Líneas separadas, sin blanco entre ellas. Nunca en horizontal ni pegadas.
- **(d) Staccato horizontal** — 3-5 items cortos como flag-block, pegados en una línea con puntos.

Reglas de oro del ritmo:
- **Línea corta por defecto:** unidad base 8-14 palabras MÁX. Si una frase pasa de ~15 o mete dos ideas con comas/"y"/"que", pártela en dos líneas cortas. Nunca encadenes frases largas.
- **No dos bloques consecutivos del mismo tipo.** Alterna: dardo, luego 2-bloque, luego dardo, luego staccato…
- **Tope de 3-bloques:** como mucho UNO o DOS 3-bloques en todo el post.
- **El cierre respira:** el final NUNCA es un muro de frases largas apiladas. Las últimas 4-5 líneas van a líneas cortas aisladas (un remate por línea).

### 3.4 · Cómo leer las peticiones de formato
- "junta más / compacta / bloques de 2" → MÁS (a), NO elimines (b)(c)(d). Quieren menos líneas sueltas INNECESARIAS, no cero.
- "más respirado / más aire" → MÁS (b), menos (a).
- "más punchy / más ritmo" → MÁS (d) en momentos clave.
...
4. **Menciones @:** solo personas que **hayan COMENTADO en el post de OTRO en los últimos 3 meses** (Iker, 2026-07-24). No basta con publicar o compartir en su propio perfil: el que solo emite y nunca comenta a nadie casi nunca nos comenta, y la mención es alcance tirado. Se comprueba con `GET {BASE}/api/v1/users/{provider_id}/comments?account_id=…`. Ordena por recencia del último comentario. Descartar a quien no comente aunque sea el CEO — un mando que comenta bate a un CEO que solo se autopublica. Además, para el CSV, se saca el **logo de LinkedIn** de cada empresa vía Unipile (no favicons).
   - **⭐ TRANSVERSAL — vale para TODOS los pilares, no solo mapas (Iker, 2026-07-24):**
     - **Una persona se menciona UNA sola vez** en el post. Repetir el mismo @ (p.ej. @Iker pegado al gancho y otra vez en la línea de resultados) gasta alcance y se lee como peloteo. Mecanizado en el validador (`Cada persona se menciona UNA sola vez`).
     - **NUNCA una mención pegada al gancho** (ni en el bloque justo después del hook). Ese espacio es para la tensión de la historia o el claim, no para etiquetar; una @ ahí se lee como cebo. Las menciones van **tejidas más adelante** en el cuerpo. Mecanizado (`Ninguna mención pegada al gancho`).
     - **La mención tiene que dejar BIEN a la persona**, aunque sea en broma. "La charla la iba a dar @Iker, me pasó el marrón a mí" deja mal al jefe Y te hace ver quejica: fuera. Menciónalo por algo que le sume, o no lo menciones.
...
- **⚠️ "PAÍS INVENTADO" NO ES UN MAPA DE PELOTEO — es OTRO formato.** No lo metas en este pilar ni cuentes su región como gastada. Diferencias de fondo: **no menciona a NADIE** (ni empresas ni personas), **la imagen NO es captura de PamPam** sino un **dibujo** en paleta Neety (país ficticio recortado dentro de España), y **el motor no es el orgullo regional sino la CONTROVERSIA**.
  - **Unai 7.87x · 78.7K** ("Nadie habla de este país…"): el país se dibujó pegado a España y sin islas en el Mediterráneo → **la gente leyó que le habíamos puesto un país encima de Baleares** y se lió. La polémica fue **involuntaria**, no de guion.
  - **Iker 1.24x · 12.7K** ("Este país no cuenta para los rankings…"): mismo formato semanas después, pero el país ficticio cayó **en un sitio donde no había nada** → nadie se sintió aludido → sin polémica, sin alcance.
  - **La firma de engagement lo delata: alcance SIN respaldo.** El 7.87x hizo 78.7K impresiones con **33 likes y 2 reposts**. Compáralo con Navarra: **79.2K impresiones y 563 likes, 46 reposts**. Mismo alcance, 17× menos likes. **La controversia compra impresiones, no cariño.** Si buscas reposts y marca, esta variante no es tu herramienta.
  - **Riesgo de marca real:** dibujar un país inventado encima de una región de verdad se lee como burlarse de ella, y eso choca de frente con `brand-voice §5` (hermandad entre regiones, jamás pinchar a otra). Que la ofensa sea un malentendido no la arregla. **Arma de ocasión, nunca la norma, y saca el trade-off en voz alta antes.**
...
- **GENIAL — País Vasco:** *"Las empresas que más vendieron este año tienen algo en común: personas desconocidas quemando el teléfono 👇"* → el comercial se ve a sí mismo → comparte.
- **FATAL — Cataluña 0.7x:** *"Las empresas catalanas que más venden salen en la foto. Quien las hace vender no sale en ninguna 👇"* → falla la identificación (frío, abstracto) Y se leyó como CRÍTICA a las empresas ("salen en la foto / no sale en ninguna" suena a reproche). Un director mencionado pidió quitar su mención.

**Reglas hardcode:**
- Hook = **verbo físico + herida propia del comercial** para que se identifique ("quemando el teléfono", "aguantando el no", "marcando nombres que nadie conoce"). El comercial tiene que poder pensar "ese soy yo". Nada abstracto.
- Hook **NUNCA** como crítica/reproche a las empresas. Este formato PELOTEA a las personas y a sus empresas, no las señala. Si suena a "las empresas no reconocen a nadie" → mata el repost y molesta a los mencionados.
...
> ## ⭐ EL FORMATO SE REPITE. LO QUE SE ITERA ES EL ÁNGULO (corregido el 2026-07-16)
> **El error que hay que no cometer:** dar por "quemada" una referencia porque ya la remixamos. **Es falso, y el mapa lo demuestra solo**: 10 iteraciones del MISMO concepto (Gipuzkoa 12.92x → Navarra 7.72x → Galicia 7.28x → Cataluña 7.11x → Valencia 6.76x…), en las 3 cuentas, y sigue petando. A nadie se le ocurre decir "el mapa está quemado": se cambia la **región** y el **concepto**, no el formato.
> **El meme va igual.** La calvicie en ventas no es un chiste de un solo uso: es una **verdad humana del sector** (como el mapa es una verdad regional). Se repite el subtipo y se itera el ángulo.
> **Lo único que NO se hace:** repetirlo **tal cual en la misma cuenta** y **pegado en el tiempo**. Cuenta distinta, meses de por medio, ángulo nuevo, imagen nueva y texto nuevo → es un post nuevo.

**SUBTIPOS DE MEME (taxonomía medida en vivo contra la BD, 2026-07-16).** Igual que PELOTEO se abre en mapa / "Los 10", el meme se abre en tres, y **no rinden ni parecido**:

| Subtipo | n | Ratio medio | Impresiones medias | Qué es |
|---|---|---|---|---|
| **A · CUERPO / PSICOLÓGICO** | 3 | **15.60x** | **157.760** | Cambio corporal progresivo (calvicie, ojeras, peso, encorvamiento, sudor) sobre una **jerarquía de roles** |
| **B · CAPTURA DOCUMENTAL** | 2 | 7.22x | 73.629 | Pantalla real creíble (iMessage, factura, X, banco) con cifra o tensión |
| **C · VIÑETA DIBUJADA** | 1 | **2.16x** | **21.761** | Dibujo/caricatura (el perro). 🔴 **NO es 0.76x: eso era a 24 h. Y hay una 2ª cifra sin resolver, ver abajo** |
...
- 🔴 **C NO ESTÁ "MUERTO CON UN 0.76x": ESA CIFRA ERA DE 24 HORAS (corregido el 2026-08-24).** Aquí ponía *"C está muerto con n=1, y ese n=1 es un 0.76x · 7.567 imp"*. Ese n=1 es el meme del perro de Unai del 15/07 (*"Tu informe de ventas resucita clientes muertos ✨"*) y la fila se rellenó el **16/07, un día después de publicarlo**. El cierre a mano del `historial-publicaciones` lo dejó en **2.16x · 21.761 impresiones · 67 clics**, que además son **117,5 clics por cada 100 interacciones, el mejor del histórico**. O sea: ni muerto ni flop, **medio**, y el mejor del pilar en conversión.
  - **La lección, que es la que se queda: un meme a las 24 horas no está medido, está empezando.** Ningún subtipo se declara muerto con menos de 7 días de reparto, y cualquier fila de esta tabla que venga de una medición de un día se vuelve a mirar antes de usarla para descartar nada.
  - **🔴 PENDIENTE, y no lo doy por bueno hasta que alguien lo mire en LinkedIn:** la BD en vivo devuelve para ese mismo post **192.303 impresiones y 14.12x**, contra las 21.761 que anotó el historial el 14/08. **Las dos cifras no pueden ser ciertas a la vez.** El resto de posts del histórico sí cuadran entre BD e historial (tatuaje 93.744, wojak 165.526, cold calling 168.926), así que la anomalía es solo de esta fila. **Hasta que se comprueben las analíticas del post, la cifra que manda es la del historial (21.761)**, que es la que se revisó a mano.
  - **Lo que sí sigue en pie pase lo que pase:** el registro de la viñeta tiene que pasar `images §0g` en Unai y Asier. **Una viñeta MONA sigue fuera de esas dos cuentas**, y el propio historial anota que el perro dibujado fue lo que disparó la regla de voz por cuenta.
...
**REGLA CLAVE nueva — texto de meme CORTO.** El motor es la IMAGEN. Cuando la imagen es el meme, el texto que la acompaña es **corto y punchy (3-6 líneas), nunca un ensayo**. (Flop: "El jefe / El líder / El comercial con IA" 0.4x — imagen de meme válida arruinada por un cuerpo largo tipo artículo.) Si el insight es largo: o cortas el texto a saco, o usas otro formato (lead magnet / mapa / insight) para la versión larga.
...
1. **Frecuencia: ⚠️ REVISADA (2026-07-14) — la "fatiga" NO existe en nuestros datos.**
   Aquí ponía: *"mín. 1 cada 2 SEMANAS por cuenta. Nunca dos seguidos en la misma cuenta; nunca solapar dos cuentas el mismo día/semana. (3 lead magnets en 3 días hundió el alcance por fatiga)"*. **Contrastado contra la BD y es falso.** La prueba que lo mata: **Iker publicó lead magnet el 27-may (0.59x · 20 com.) y OTRO el 28-may (8.52x · 483 com.)** — días consecutivos, misma cuenta, y el segundo es el 2º mejor post del histórico. Con fatiga eso es imposible.
   **Los 5 flops de junio se explican TODOS por ángulo o mecánica, ninguno por frecuencia:** arsenal 0.64x y biblia 1.20x (entregable genérico, ya muerto) · "destripo" 0.21x (verbo pasado, §2.9) · "traduzco comentarios" 0.43x (sin valor real) · 18-jun 0.57x (el CTA pedía "+ tu mes de cumpleaños", variante ya descartada por tono).
   **Regla nueva:** no hay puerta de frecuencia dura. **Un lead magnet flopea por su ÁNGULO y su MECÁNICA, no por cuándo se publicó.** Puedes tener lead magnet en las 3 cuentas la misma semana en días distintos (es lo que exige la rotación 3×3 de `post-workflow §8.2`). Lo mismo pasa con los mapas: Unai el 28-abr (3.69x) e Iker el 30-abr (7.11x), dos días de diferencia y los dos bien.
   **Lo único que sí vigila:** que el ángulo no se parezca al del último de esa cuenta, y no repitas la misma PALABRA de CTA. Si dudas, la energía va en el ángulo, no en el calendario.
...
**🚨 Y un tercer sitio donde el UTM se evaporaba, encontrado el mismo dia:** las guias con puerta (`/senales/guia/`, `/llaves/guia/`…) devuelven un **302 a la landing** cuando no traes la cookie del gate, y ese redirect **tiraba la query entera**. Un post que enlazara la URL de la guia perdia sus `utm_*` en el salto, antes de que ninguna pagina pudiera capturarlos. Arreglado en `src/gate.js`: el destino ahora arrastra `url.search`. **La leccion general: cada redirect nuestro es un punto donde la atribucion se puede caer sin ruido**, y hay que comprobarlo con `curl -o /dev/null -w "%{redirect_url}"` antes de dar por bueno un enlace.
...
>
> **NEETY FORWARD** · `https://luma.com/ujffj66o` · **24 de septiembre, 17:30, Donostia** · **80 plazas**, privado y por solicitud · se anuncia **la nueva generación de Neety** en primicia. **Talent House es la SEDE**, no el producto: me confundí el 04/08. Puertas 17:30, arranque 18:00, y **las solicitudes se revisan una por una**, que es lo que sostiene el FOMO.
>
...
>
> **🔗🔗 Y EL ENLACE DE LUMA VALE EN CUALQUIER PILAR, NO SOLO EN UN POST DE EVENTO (2026-08-21).** Es el corolario del aviso de aquí arriba —*un post informativo del evento no lo lee nadie*— y estaba sin escribir: si el evento **viaja dentro** de una historia, un meme o un mapa, **el enlace que viaja es el de Luma y ocupa el hueco del de agendar**, con las reglas de forma del ninja intactas (`§4.4b`). No se apilan las dos puertas de conversión en el mismo post.
> - **Mecanizado el 21/08**, porque el validador no lo contemplaba: los cuatro checks del dominio estaban atados a `--pilar evento` y tumbaban la historia de Unai del 21/08, que es exactamente como manda venderlo. Ahora lo que decide es que **el único enlace de fuera sea el de Luma**, sea cual sea el pilar.
> - ⚠️ **Y lo que cuesta, dicho: ese post ya no mide clics a `/agendar/`**, así que no entra en la casilla de `§4.4e` ni en la vara de CTR del pilar. Se mide contra el propio Luma.
>
...
> **⚠️ MATIZ (Iker, 2026-08-12): eso vale cuando la fuente se puede CITAR en el post.** Si la cifra es interna y no lleva fuente al lado, una redonda real se lee fake igual, porque el lector no puede distinguirla de una inventada. Caso: el lead magnet de Unai decía *"lo vimos claro en 50 reuniones"* — las 50 eran reales (informe de demos de julio) y aun así Iker la quitó: *"parece fake"*. Se cambió por el cualitativo (*"en muchas reuniones"*). **La escalera: cifra impar con fuente citable > cualitativo llano > cifra redonda sin fuente.**
...
> **Lo que se DESCARTÓ midiendo, no opinando:** no es la herramienta (el mismo parser lee 230 en Navarra y 0 en Murcia, y saves/sends/botón cuadran exactos con la BD) · no es el enlace roto (Iker lo abrió y lleva bien a la web) · no es el dominio (`recursos.neety.com/agendar/` sí cuenta) · no es la colocación (los tres mapas llevan el enlace en la misma línea, la 60 de 64) · **no es una edición posterior (Iker confirma que no editó el post de Murcia)**. La causa última está dentro de LinkedIn y no es observable desde fuera.
...
**Las 7 reglas duras:**
1. **MÁX 2 líneas CORTAS, FUSIONADAS: la broma pegada al CTA y al enlace** (Iker, 2026-07-22). Nada de [broma] → blanco → [CTA genérico "te marcamos quién compra y cuándo, tu comercial entra solo ahí"]: eso deja la broma suelta y el CTA suena a plantilla. Van **juntas, máx 2 líneas, con el enlace dentro**, y la 2ª línea **responde a la broma** (no es un CTA de catálogo). El "te marcamos… tu comercial entra solo ahí" es **demasiado genérico y largo**: se acorta a un remate que conecte con el chiste (ej. broma *"Otra herramienta no te dice en cuál picar ni cuándo."* → remate *"Nosotros sí: [link]"*). Y cuenta el ANCHO, no los puntos: dos líneas kilométricas envuelven en móvil y cantan a anuncio → mantenlas cortas.
```
❌ MAL                                    ✅ BIEN
...
3. **Reenlaza con el CONCEPTO del hook, girándolo.** No repitas la frase del gancho: coge su palabra y úsala para describir el dolor de ventas. Validado (Aragón): hook de "secarral" → *"Llamar a 500 a puerta fría para que compre uno. Eso sí que es un secarral."*
   > **📊 MEDIDO 2026-07-22 (tentativo, re-medir) — reciclar la palabra-cliché del gancho es lo que más CTR da.** De los 10 posts con enlace de agendar y clics medidos, los **2 de mayor CTR** son los que meten la **palabra-cliché exacta del hook** en el chiste: Aragón *"…que compre uno. Eso sí que es un **secarral**"* (0,41% · 112 clics) y Cataluña *"…que te compre 1. Eso sí que es una **playa**"* (0,38%). El "Los 10" de Asturias lleva la misma estructura "500→3" y la misma fórmula de cierre pero con dolor **genérico** ("desgaste", no el cliché) y cae a 0,10%. **La fórmula ganadora completa:** `[dolor "500 a puerta fría para uno"] + [cliché EXACTO del hook] // Te marcamos quién compra y cuándo. Tu comercial entra solo ahí: [link]`. ⚠️ **Caveats:** n=2 arriba; el CTR de Cataluña está inflado (solo 4.7K impresiones, y el post fue flop 0.59x); y **el CTR NO se compara entre pilares** — los memes (perro 0,05%, iMessage 0,09%) salen bajos solo porque LinkedIn les infla las impresiones (`outliers §3.11`), pero en clics ABSOLUTOS son de los que más (96, 76). Solo tiene sentido comparar CTR dentro del mismo pilar.
...
| **MEME** | agendar | ⚠️ **OPCIONAL, y el presupuesto se mide SIN él** (ver `§4.4e-MEME`). ✅ **La doble puerta NO capa** — refutado el 20/08 con el meme de Asier, mismos dos enlaces y se repartió (`post-workflow §4.5.0-PUERTAS`) | El chiste sigue teniendo sus 450 caracteres; lo que ocupa la puerta nueva no se le resta. Nunca se recorta el chiste para meter el segundo enlace. |
...
- Pico de leads esta semana + entregable real → LEAD MAGNET.
- Alcance amplio + reposts + idea meme-able → MEME (con motor).
- Alcance + identidad de marca + ángulo regional/industrial → MAPA.
- Celebrar a los comerciales infravalorados de una región → "LOS 10".
- **Cuando el objetivo es PIPELINE (no solo alcance):** sesga hacia "dolor de mercado concreto + pregunta que apunta a la categoría de producto" (16.6x "caja de herramientas engorda") — es la forma que mejor convierte atención en atención relevante para producto. Los posts de OFERTA DE EMPLEO son un mecanismo fiable de alcance+reposts de gama media (2.9x · 15 reposts).
- La idea no encaja en ninguna → ejecución limpia del arquetipo, sin bolt-on de mecánica.
...
## 6 · Qué está funcionando / qué se está quemando (diagnóstico vivo)

- **Solo 2-4 mecánicas fiables ahora mismo:** mapa (región underdog), meme con motor, "Los 10", y lead magnet (con sus puertas). Los formatos "bonitos sin motor" (mockup Amazon-CV 0.8x, póster vintage 0.4x, escena ilustrada estática 0.7x) caen SIEMPRE a 0.4-0.8x. Experimenta formatos nuevos con expectativa de alcance baja; no apuestes una semana a "bonitos" sin probar.
- **🔴 EL LEAD MAGNET LLEVA SEMANAS FLOPEANDO EN LAS 3 CUENTAS (Iker, 2026-07-22, con datos).** Los recientes: Iker `auditoria` 0.29x, Unai `mensajes` 0.57x, Asier `comité` 0.45x, Iker `biblia` 1.20x, Unai `destripo` 0.21x, `arsenal` 0.64x. Los buenos (vibe 9.85x, desmonto 8.52x, llaves 4.52x) son de **mayo**, no recientes. **Mapa, "Los 10" y meme SIGUEN yendo bien.** Consecuencia práctica: al planificar, **sesga hacia mapa/meme/"Los 10"** y mete lead magnet con **expectativa baja** hasta entender por qué cae (o hasta un remix que de verdad reutilice la esencia, `global §2.2b`). No es que el formato esté muerto — es que lo que estamos metiendo dentro no engancha. Y **explorar pilares nuevos** (incluidos los de marketing para la cuenta de Mario, `aboutme §2`).
- **Posts de EVENTO / premio:** 0.2x-0.8x es NORMAL y esperado. Objetivo = autoridad, no viralidad. Avisar antes de crearlos. Imagen: fotos naturales/selfie con caras de los jefes, grupo, microfonados, detalle (vino en mano).
- **Retirar (no proponer salvo insistencia):** "El problema eres tú" / "Y nadie te lo dice" (3 flops). Listicles de herramientas IA sin mecánica ("6 IAs que uso…"). Infografías (0/3). Listicles con negrita Unicode (𝗟𝗮𝘀 𝟭𝟬…). Posts de lanzamiento de modelo Claude/IA (0.88x, 0.25x — reformular como resultado en términos de la audiencia). CV-Spotify / variantes de CV genérico (0.3x — no conectan). "Spoiler:"/"P.D." de cierre.
- **Vibe prospecting / Claude + outbound:** aún pega (9.7x, 4.95x) pero la curva decae — pausa 7-10 días entre uno y otro.
...
4. **Spam ninja (§4.4b):** ¿está? ¿≤2 líneas CORTAS (mide el ancho, no los puntos)? ¿NO nombra a Neety? ¿gira el concepto del hook? ¿verbo punchy sin pasarse? ¿colocado donde toca y NUNCA de última línea? ¿lead magnet → sin spam ninja?
5. **Fechas de fuente:** ¿el cuerpo cita el NOMBRE de la fuente y NINGÚN año? (`(IAEST)` sí, `(IAEST 2024)` no, "vendieron X **en 2025**" tampoco). Los años van en la entrega interna, fuera del post.
6. **Ángulos quemados:** ¿el cuerpo/cierre repite el eje "callado / en silencio / no lo cuentan / nadie los conoce"? En MAPAS está **prohibido** (`post-workflow §4.2` Paso 3) — rota a otro eje de mérito. (En "Los 10" ese eje sí es el suyo.)
7. **Frase-rabia:** en mapas, ¿el hook lleva el beat "y poco más" **reformulado** (nunca literal, nunca ausente)?
...
- **⛔ ANTES DE BORRAR, SE ANOTAN LOS NÚMEROS (2026-08-26).** Al borrar el post desaparecen sus impresiones, su hora y su pendiente, **y con ellos la única prueba de que estaba capado**. El 26/08 se borró el original y lo que quedó fue lo que Iker había dicho de memoria (*45 impresiones a los 14 minutos*); una medición programada que iba a dar la pendiente **acabó midiendo el post nuevo**. **Se apunta en la ficha: impresiones y minuto de cada lectura, minutos hasta aparecer en Principal, y la hora exacta de publicación.** Cuesta 30 segundos y es lo único que hace que el siguiente capado se diagnostique con datos en vez de con memoria.
