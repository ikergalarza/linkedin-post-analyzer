...
- [ ] 🔴 **FOTO DEL REMITENTE — avisar a Mario en CADA correo hasta que se resuelva (pedido por él, 2026-08-06).** Hoy los correos salen con la inicial genérica de Gmail en vez de un icono. **Dos caminos distintos según el tipo de envío, aclarado el 2026-08-18:**
  ...
  - **Envío INDIVIDUAL (prospección, respuestas a mano, uno a uno):** SÍ funciona con **Google Workspace**, gratis, PERO solo si `@neety.com` ya está en Workspace de verdad. Requiere que esa dirección sea una **cuenta real con login**, no un alias ni un reenvío (un reenvío de registrador no crea cuenta de Google, no hay dónde subir foto). Se sube en `myaccount.google.com`, y solo se ve cuando el correo sale directo de Gmail/Workspace — nunca en un envío por lotes de un ESP. Tarda de horas a un par de días en propagar y no está garantizado con un desconocido sin interacción previa.
  - **⚠️ Comprobado el 2026-08-18: el dominio va por reenvío, no por Workspace real.** Pasar a Workspace exige cambiar el **MX del dominio entero** (no se puede activar solo para una dirección) — es un cambio de DNS de la misma categoría que el subdominio de Brevo (`§0c`), así que se coordina con ese mismo cambio y no por separado. Y es coste recurrente por usuario: es una decisión de si compensa, para llevar a Iker, no una tarea de 5 minutos.
