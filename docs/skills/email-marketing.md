...
- **Y no vale para todos los asuntos por igual.** `no te acuerdas de mí?` (18 car) funciona corto porque juega con psicología emocional pura: no necesita contexto, la frase entera es el gancho. Un asunto de historia (`traje 200 tarjetas y no llamé a ninguna`, 39 car) necesita más recorrido porque el gancho es narrativo — cortarlo de más le quita la escena. **El objetivo es no pasarse de largo innecesariamente, no perseguir el mínimo.**
