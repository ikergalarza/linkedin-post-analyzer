...
1. **Tandas de 2 semanas.** Se planifica y escribe la tanda completa de una vez (con la rotación de remitentes y pilares), no email a email. Un email suelto fuera de tanda es la excepción (algo urgente que comunicar), y va sin bucles con ayer/mañana.
...
- **NUNCA inventar una empresa, una persona, un dato ni un CASO DE ÉXITO.** ⚠️ Carmen contó que en Runner Pro inventan historias de casos de éxito tocando dolores reales. **Eso aquí está PROHIBIDO** (CLAUDE.md, innegociable nº 1): en B2C con una app de running cuela; en B2B el prospecto puede pedir hablar con ese cliente y el castillo se cae en la primera demo. Lo no verificable va como `[PENDIENTE · no verificado]` o se cuenta como **prueba lógica** (el proceso, el flujo, el antes/después mecánico) sin nombre propio.
...
- **Autoridad por cicatrices, no por púlpito:** sus mejores correos son fracasos propios con autopsia numerada. Neety tiene el equivalente REAL: campañas de prospección que fallaron, señales mal leídas, listas mal cualificadas. Con dato real, que aquí no se inventa (§7).
...
- [ ] **Herramienta de envío:** quedó abierto en la reunión con Carmen (¿Klaviyo de arranque ~50€/mes y migrar luego al CRM propio, o directo al CRM propio?). Sin esto no hay tanda programable ni métricas.
