...
- [ ] 🔴 **FOTO DEL REMITENTE — avisar a Mario en CADA correo hasta que se resuelva (pedido por él, 2026-08-06).** Hoy los correos salen con la inicial genérica de Gmail en vez de un icono. **Dos caminos distintos según el tipo de envío, aclarado el 2026-08-18:**
  ...
  - **⭐ CORREGIDO el 2026-08-18: NO hace falta Workspace ni tocar el MX del dominio. Mario ya lo probó gratis para sí mismo.** El truco real: crear una **cuenta de Google independiente** que use la dirección `@neety.com` como identidad, sin que eso la convierta en un buzón ni toque el dominio.
    ...
    - 🔴 **INCIDENTE 2026-08-18: `hola@neety.com` se quedó SIN RECIBIR NINGÚN CORREO** (`Address not found`) al desmontar el Grupo de Google que hacía de reenvío, antes de tener el reemplazo funcionando. El dominio de la empresa vive en **Cloudflare** (no Porkbun, donde el mismo truco personal de Mario sí funcionó a la primera): **Cloudflare Email Routing solo permite UN destino por regla desde su interfaz oficial**, por eso el Grupo de Google era el mecanismo para repartir a varias personas. Al borrarlo sin dejar nada verificado en su lugar, la regla se quedó rota y todo empezó a rebotar.
      - **Checklist de diagnóstico** (lo ejecuta quien tenga acceso a Cloudflare, Mario no lo tiene): 1) Destination addresses → el destino tiene que estar VERIFICADO (tick verde) · 2) Routing rules → tiene que existir una regla activa para `hola`, no borrada ni en borrador · 3) el interruptor general de Email Routing tiene que estar en ON (si no, no hay MX de Cloudflare gestionando el correo) · 4) DNS records → que no haya un MX antiguo compitiendo con el de Email Routing.
