...
| **6 · 🆕 Datos o conversaciones de TERCEROS** | `la transcribo`, `grabé la llamada`, `mi transcripción`, `rastrear`, `encontrar a alguien` | **La más peligrosa que tenemos, porque roza lo que vendemos.** Lo que capa no es la palabra: es **reclamar en PRIMERA PERSONA haber grabado o transcrito a un tercero**. Capó el meme del 19/08 a **133 imp**, y el mismo post con `apuntar` voló. ⚠️ **En impersonal NO capa:** `6️⃣ Grabar la llamada` como ítem de lista hizo **165.526** (nuestro mayor meme) y `12 transcripciones de discovery` hizo 4.082. **El test: ¿esta frase, leída por un abogado y sin contexto, suena a que hemos cogido datos de alguien que no nos los dio?** Recambios: `apuntar`, `dejar por escrito`, `pasar a papel`. ⛔ `grabar` NO vale de recambio. La palabra sí puede vivir en la FOTO, que LinkedIn no lee. Mecanizado. Caso entero en `post-workflow §4.5.0-DATOS` |
